...
!
! Changed sjs
! 		emissions and results passed via arrays
! 		toggle added not to writeout results
!		changed to double precision reals
!		--- Must change all procs AND BLOCK DATA, add integer and real statement

! Changed hp	(5/02)
!		made all the alog's into dlog's
!		reorganized the common blocks so all the integer variables come at the end
!			(removes alignment warnings)
...
!
	...
	REAL*8 MAGICCCResults(0:30,50), MagEM(20,16)	! sjs - new vars to pass data. 02/02
	Integer IWrite
...
!
      common /Limits/KEND
!
      COMMON/CLIM/DT,DZ,FK,HM,Q2X,QXX,PI,T,TE,TEND,W0,XK,XKLO,&
...
!
      ...
      FL1,FL2,FL3,XL,GAMH,GAMS,QS0,BTSOIL,FERTTYPE,TOTEM, &
      R(4),CPART(4,5),DELMASS(4,226:iTp),ABFRAC(4,226:iTp),CONVTERP 
!
      COMMON /METH1/emeth(226:iTp),ch4l(225:iTp),ch4b(225:iTp), &
      ch4h(225:iTp),ef4(226:iTp),StratH2O,TCH4(iTp),ednet(226:iTp+1), &
      DUSER,FUSER,CORRUSER,CORRMHI,CORRMMID,CORRMLO,imeth,iO3feed
!
      COMMON /FORCE/qco2(0:iTp),qm(0:iTp),qn(0:iTp),QCFC(0:iTp), &
      QCFCBIT(0:iTp),QSTRATOZ(0:iTp),QCH4O3(0:iTp)
!
      COMMON /METH2/TCH4CON,TauSoil,DELTAU,TAUINIT, &
      ch4bar90,eno90,eco90,evo90,powc0,delpowc,powco0,delpowco, &
      powno0,delpowno,powvo0,delpowvo,QQQN2O,IOLDHALO,LEVCH4
!
      common /TauNitr/TAUN2O
!
      common /Sulph/S90DIR,S90IND,S90BIO,S90OZ,enat,ES1990, &
      dqdcoz,QCH4OZ,IGHG
      COMMON /CO2READ/XC(226:iTp),CO2SCALE,qtot86,ICO2READ,LEVCO2
      COMMON /DSENS/XLAML,XLAMO,ADJUST,IXLAM
...
!
      COMMON /QADD/QEX(0:iTp),QEXNH(0:iTp), &
      QEXSH(0:iTp),QEXNHO(0:iTp),QEXNHL(0:iTp),QEXSHO(0:iTp), &
      QEXSHL(0:iTp),IOLDTZ,IQREAD,JQFIRST,JQLAST
!
      COMMON /NSIM/TEMEXP(2,40),WTHRESH,NSIM,NCLIM,ISCENGEN,IWNHOFF,IWSHOFF
...
!
      IF (IWrite .eq. 1) OPEN(UNIT=8,FILE='..\magicc2\MAG.OUT',STATUS='UNKNOWN')
!
!  ************************************************************
!
...
! Now Skip this read. Get emissions directly from input array. sjs 02/02

IF (1 .eq. 2) THEN	!Toggle
...
! New code that extracts emissions from input array
! Based on mrj code above
! Turn this block off if want to read from file
...
IF ((fos(i) .gt. 20 .or. NVAL .gt. 9) .and. 1.eq.2) &
Write(*,854) "MAG: ",NVAL,IY1(i),fos(i), def(i), DCH4(i), DN2O(i), &
        DSO21(i),DSO22(i),DSO23(i),DCF4(I),DC2F6(I),D125(I), &
        D134A(I),D143A(I),D227(I),D245(I),DSF6(I)
...
!IF (NVAL .ge. 9) Write(*,*) "Nvals & Lastyear: ",NVALS, LASTYEAR, KEND
! IYEND is = LASTYEAR

!xx   END DO ! Timing Loop Ixxyy

!    ReadTime =  (secondsn() - StartTime)
!   	Write(*,*) "ReadTime ",ReadTime," seconds"
!   	Write(97,*) "ReadTime ",ReadTime," seconds"
 
!    T_StartModel = secondsn()
...
IF (IWrite .eq. 1) THEN	! sjs skip if write turned off
...
!
      IF(KYEAR.GE.IY0)THEN
        ...
        IF(KKKK.EQ.1 .and. IYEAR .eq. 1)THEN
!
...
!
      ...
      IF(ISCENGEN.EQ.1 .and. IWrite .eq. 1)THEN
!
        DO NCLIM=1,4
        DO K=197,KEND
          KSG=K-196
...
IF (IWrite .eq. 1) THEN	! Skip if write turned off
...
!          IF(IO3FEED.EQ.0)DELSTROZ=0.0
	 ...
	 MAGICCCResults(10,(K-1990)/IIPRT+1) = DELQDIR
	 ...
	 MAGICCCResults(18,(K-1990)/IIPRT+1) = ESO21(IYR)
	 MAGICCCResults(19,(K-1990)/IIPRT+1) = ESO22(IYR)
	 MAGICCCResults(20,(K-1990)/IIPRT+1) = ESO23(IYR)
...
!    ModelTime =  (secondsn() - T_StartModel)
!	Write(*,*) "ModelRun ",ModelTime," seconds"
!	Write(97,*) "ModelRun ",ModelTime," seconds"
...
! debugging. sjs    
        IF (1.eq.2) Write(*,'(2I4," -",10f5.0," | ",10f5.0," | ",10f5.0)') J,NC, &
        TEMP,EF4(J),EDNET(J),CMINUS3,CMINUS2,CMINUS1, &
        PL(NC,J-1),HL(NC,J-1),SOIL(NC,J-1),REGROW(NC,J-1), &
        ETOT(NC,J-1), &
        PL(NC,J)  ,HL(NC,J)  ,SOIL(NC,J)  ,REGROW(NC,J)  ,ETOT(NC,J)  , &
        ESUM(J),FOC(NC,J),DELMASS(NC,J),EDGROSS(NC,J), &
        CCO2(NC,J)
...
!
      IF(ICO2READ.EQ.1.AND.J.GT.226)THEN
        QTOT(J)=QTOT(226)+CO2SCALE*(QXX*DLOG(CO2(J)/CO2(226)))
        QSO2(J)=QSO2(226)
        QDIR(J)=QDIR(226)
        QOZ(J) =QOZ(226)
        QBIO(J)=QBIO(226)
        QGH(J) =QGH(226)+CO2SCALE*(QXX*DLOG(CO2(J)/CO2(226)))
      ENDIF
!
...
!
        CB=31.
        R(MM)=(1.+BCO2(MM)*DLOG(680./C0))/(1.+BCO2(MM)*DLOG(340./C0))
        AR=680.-CB
        BR=340.-CB
        BEE=999.9
...
!
      ...
      DO J=1,5
         ...
         FLUX=SUMEM1-FACTOR*DELC
         AA(MM,J)=A1(MM,J)
         IF (1.eq.2) Write(*,'("carb loop bits: ",I4," -",4f5.0," | ",5f5.0," | ",10f5.0)') J, &
            z, TINV(MM,J), DELA, ABAR , &
            A1(MM,J), AA(MM,J), A(1:3,J), &
            EE, XXX4, XXX3, EL21,EL32
...
! debugging. sjs    
        IF (1.eq.2) Write(*,'("carb: ",I4," -",10f5.0," | ",10f5.0," | ",10f5.0)') MM, &
        TEM,EFOSS,ENETDEF,CPP,CPREV,C,PL ,HU ,SO ,REGRO, &
        ETOT, PL1,HU1,SO1,REGRO1,ETOT1,SUMEM1,FLUX,DELM,EGROSSD, &
        C1
 
        IF (1.eq.2) Write(*,'("carb bits: ",I4," -",3f5.0," | ",4f5.0," | ",10f5.0)') MM, &
        SUMEM1, FACTOR, DELC, &
        ABAR, SUMEM1, Z, CPART(MM,J), &
        TINV(MM,J), DELA
...
!
        ...
        qindir = s90ind*(dlog(1.0+e/ENAT))/(dlog(1.0+ES1990/ENAT))
!
...
!
      REAL*8 LAMO(100),LAML(100),LAMOBEST,LAMLBEST,LAM,KLO,KNS
!
      KLO=XK/2.0
      KNS=XKH/2.0
      FNL=FNHL/2.0
