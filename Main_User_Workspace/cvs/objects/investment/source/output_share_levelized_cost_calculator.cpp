...
/*! 
 * \file output_share_levelized_cost_calculator.cpp
 * \ingroup Objects
 * \brief OutputShareLevelizedCostCalculator class source file.
 * \author Pralit Patel
 */
...
//! Constructor
OutputShareLevelizedCostCalculator::OutputShareLevelizedCostCalculator(){
}

/*! \brief Return the XML name for this object as a string.
* \return The XML name for the object.
*/
const string& OutputShareLevelizedCostCalculator::getXMLNameStatic(){
    const static string XML_NAME = "output-share-levelized-cost-calculator";
    return XML_NAME;
}
...
* \param aTabs The object which tracks the number of tabs to write.
*/
void OutputShareLevelizedCostCalculator::toDebugXML( const int aPeriod, ostream& aOut, Tabs* aTabs ) const {
    XMLWriteOpeningTag( getXMLNameStatic(), aOut, aTabs );
    XMLWriteClosingTag( getXMLNameStatic(), aOut, aTabs );
}

/*! \brief Calculate the parent level levelized cost.
* \details Uses an Investables annual investemtn to determine it's share
...
* \param aInvestmentLogitExp The investment logit exponential.  Not used here.
...
* \warning This method should only be used after investment has been
*          distributed.
*/
double OutputShareLevelizedCostCalculator::calcSectorExpectedProfitRate( const vector<IInvestable*>& aInvestables,
...
{
    // should not be called until after distribution is done
    assert( !aIsDistributing );

    // Sum levelized cost for the subsector/sector with a share based on annual investment
    double totalInvestment = 0;
    vector<double> levelizedCosts;
    vector<double> annualInvestments;
        ...
        currInv != aInvestables.end(); ++currInv )
    {
        ...
        if( currLevelizedCost > 0 ){
            levelizedCosts.push_back( currLevelizedCost );
            double currInvestment = (*currInv)->getAnnualInvestment( aPeriod );
            annualInvestments.push_back( currInvestment );
    ...
    for( unsigned int i = 0; i < levelizedCosts.size(); ++i ) {
        double share = annualInvestments[ i ] / totalInvestment;
        sectorLevelizedCost += share * levelizedCosts[ i ];
    }
...
*/
double OutputShareLevelizedCostCalculator::calcTechnologyExpectedProfitRate( const ProductionFunctionInfo& aTechProdFuncInfo,
                                                                  const NationalAccount& aNationalAccount,
                                                                  const string& aRegionName,
