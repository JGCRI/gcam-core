...
/*! 
* \file rcp_forcing_target.cpp
* \ingroup Objects
* \brief RCPForcingTarget class source file.
 ...
 */
RCPForcingTarget::RCPForcingTarget( const IClimateModel* aClimateModel,
 ...
 */
const string& RCPForcingTarget::getXMLNameStatic(){
    static const string XML_NAME = "rcp-forcing-target";
    return XML_NAME;
}
 ...
 */
double RCPForcingTarget::getStatus( const int aYear ) const {
    ...
    const double currForcing = mClimateModel->getForcing( "RCP", year );
    ...
    targetLog << "Currently " << percentOff << " percent away from the RCP forcing target." << endl
              << "Current: " << currForcing << " Target: " << mTargetValue << " In year: " << year << endl;

    return percentOff;
}

int RCPForcingTarget::getYearOfMaxTargetValue() const {
    ...
    for( int year = mFirstTaxYear; year <= finalYearToCheck; ++year ) {
        if( maxForcing < mClimateModel->getForcing( "RCP", year ) ) {
            maxForcing = mClimateModel->getForcing( "RCP", year );
            maxYear = year;
        }
    }
    return maxYear;
