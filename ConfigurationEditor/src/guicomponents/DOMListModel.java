...
public class DOMListModel extends AbstractListModel implements
    ...
    public boolean contains(Object aMatchObject) {
        ...
        Element newElement = DOMUtils.createElement(mRoot, mElementName, aMatchObject, mChildrenAreLeaves);
    ...
    public Object getElementAt(int aPosition) {
        ...
        if(DOMIndex != -1) {
            return NodeWrapper.createProxy(mRoot.getChildNodes().item(DOMIndex));
        }
    ...
    public void addElement(Object aObject) {
        ...
        Element newElement = DOMUtils.createElement(mRoot, mElementName, aObject, mChildrenAreLeaves);
        ...
        int addedItemListIndex = DOMUtils.getListIndexOfObject(mRoot, newElement);
    ...
    public int moveElementBack(Object aObject) {
        ...
        fireContentsChanged(this, DOMUtils.getListIndexOfObject(mRoot, curr), DOMUtils.getListIndexOfObject(mRoot, prev));
    ...
    public int moveElementForward(Object aObject) {
        ...
        Node prev = DOMUtils.getItemAfter(mRoot, DOMUtils.getItemAfter(mRoot, curr));
        curr = mRoot.removeChild(curr);
        mRoot.insertBefore(curr, prev);
        fireContentsChanged(this, DOMUtils.getListIndexOfObject(mRoot, DOMUtils.getItemBefore(mRoot, curr)),
    ...
    private void update(int aNewIndex) {
        ...
        // Create the node for the list if there isn't one already.
        if (result == null) {
            result = DOMUtils.addNodesForXPath(mDocument, mParentSelectedItemXPath);
        }
        mRoot = result;
        fireContentsChanged(this, 0, getSize() - 1);
    }
