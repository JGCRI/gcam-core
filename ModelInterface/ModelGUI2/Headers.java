...
public class Headers {
	...
	public Headers(String[] splitHeaders) throws Exception {
		...
		for (int i = 0; i < splitHeaders.length; i++) {
			...
			/*
			 * if(splitHeaders[i].matches(".*->.*")) { parent =
			 * splitHeaders[i].substring(splitHeaders[i].indexOf(']')+1,
			 * splitHeaders[i].indexOf('-')); firstArg =
			 * splitHeaders[i].substring(splitHeaders[i].indexOf('[')+1,
			 * splitHeaders[i].indexOf(']')); secArg =
			 * splitHeaders[i].substring(splitHeaders[i].indexOf('>')+1);
			 * System.out.println("Got a Map thinger: "+parent+" reads
			 * "+firstArg+" maps to "+secArg); numCols--; continue; }
			 */
			...
			if (st.countTokens() == 1) {
				...
				parent = st.nextToken() + "/" + st.nextToken() + "/"
						+ st.nextToken();
				...
				child = null;
				parent = null;
				throw new Exception("Invalid Formated Header: "
						+ splitHeaders[i]);
				//System.out.println("Invalid Formated Header:
				// "+splitHeaders[i]);
				//System.exit(0);
			}
			if (child.startsWith("+{") && (child.indexOf("=") == -1)) { // just
																		// attr
				...
				parentAttrMap.put(parent + "\\" + child + "\\" + firstArg,
						new Integer(i));
				//attrMap.put( child, new Integer( i ) );
			}

			else if (child.startsWith("+{")) { // attr and data, should start
											   // with '+{'
				stA = new StringTokenizer(child, "=", false);
				...
				tagAttrMap.put(child, firstArg);
				if (!parentAttrMap.containsKey(parent + "\\" + child + "\\"
						+ firstArg)) {
					tempArr = new ArrayList();
				} else {
					tempArr = (ArrayList) parentAttrMap.get(parent + "\\"
							+ child + "\\" + firstArg);
				}
				tempArr.add(secArg);
				parentAttrMap.put(parent + "\\" + child + "\\" + firstArg,
						tempArr);
				dataMap.put(parent + "\\" + child + "\\" + firstArg + "\\"
						+ secArg, new Integer(i));
				...
				/*
				 * if (parent.matches(".*\\..*")) { System.out.println("HERE in
				 * GP"); parentPlaceHolder = parent; parent =
				 * parent.substring(parent.indexOf('.')+4);
				...
				if (parent.startsWith("{")) {
					...
					parentAttrMap.put(parent + "\\" + child + "\\" + firstArg,
							tempArr);
					//parentAttrMap.put(parent + "\\" + child + "\\" +
					// firstArg, secArg );
					//parentAttrMap.put(child + "\\" + firstArg, secArg );
				}
				if (parentPlaceHolder != null) {
					System.out.println("Switching back from " + parent + " to "
							+ parentPlaceHolder);
		...
		for (int i = 0; i < parentTemp.size(); i++) {
			...
			if (temp != -1 && !((String) childArr.get(temp)).startsWith("*")) {
				...
				// "+(String)childArr.get(temp)+" at "+temp);
	...
	// pre: assumes headers data is already read in and only one of the headers
	// is the root.
	...
	// Given "World", returns an array of all the indices where "world" is the
	// first part
		...
		for (int i = 0; i < numCols; i++) {
			if (parentArr.get(i) != null) {
				...
				if (strToTest.matches(".*\\.\\..*") && dbgChk) {
					System.out
							.println("Child stack: " + checkChildStackFront());
					System.out.println("the parentStr: " + strToTest);
					System.out.println("substred: "
							+ strToTest
									.substring(strToTest.lastIndexOf('/') + 1));
				}
				if (i == checkChildStackFront()
						&& strToTest.substring(strToTest.lastIndexOf('/') + 1)
								.equals(word)) {
					if (dbgChk) {
						System.out.println("In some deal about child front: "
								+ i);
					}
					returnvalue.add(new Integer(i));
					continue;
				}
				if (strToTest.split("/").length > 1
						&& checkChildStackFront() == -1) {
