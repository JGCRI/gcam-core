...
First, let's edit the input, `FAO_municipal_water_AQUASTAT.csv` by duplicating its final row. This adds another value of 3.28 to the file. The values from this file get aggregated, so adding a non-zero value affects other chunks. 
...
* `history`: logical, whether to record the build history of targets (default: `TRUE`). This is helpful if you need to recover old data. Or perhaps check how some outputs changed between commits. However, given the size of the data produced by `gcamdata` it may lead to very large cache sizes.  Thus it may be benefical to set it to `FALSE` or at least clean out the cache from time to time.

<!-- TODO: check what the maximum memory usage is when using "speed" to give reccomentation below -->
* `memory_strategy`: character scalar, name of the strategy `drake` uses to load/unload a target's dependencies in memory (default: `"speed"`). Some options include
    * `"speed"`: maximizes speed but hogs memory. Recommended for users with (give recommendation for users who are memory constrained)
...
For using `clustermq` on PIC (PNNL institutional computing) we have already installed the full set of required R packages in a shared space as succesfully compiling `clustermq` was not straightforward due to compiler version issues.  To use `drake` + `clustermq` in PIC a user can:
    * Get necessary libraries with: `export R_PATH=/pic/projects/GCAM/GCAM-libraries/R/x86_64-pc-linux-gnu-library/3.5`
...
vis_drake_graph(plan, from = gcamata:::sanitize_drake_target_name("L210.RenewRsrc"))
