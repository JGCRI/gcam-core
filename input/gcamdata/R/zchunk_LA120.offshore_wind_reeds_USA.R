...
module_energy_LA120.offshore_wind_USA <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "gcam-usa/reeds_regions_states",
             ...
             FILE = "energy/reeds_offshore_wind_curve_grid_cost",
             FILE = "energy/reeds_offshore_wind_curve_CF_avg"))
    ...
    resource.potential.EJ <- fixed.charge.rate <- technology <- socioeconomics.FINAL_HIST_YEAR <- fcr <- OM.fixed <- price <- CFmax <- supply <- curve.exponent <-
    ...
    reeds_offshore_wind_curve_grid_cost <- get_data(all_data, "energy/reeds_offshore_wind_curve_grid_cost")
    reeds_offshore_wind_curve_CF_avg <- get_data(all_data, "energy/reeds_offshore_wind_curve_CF_avg")
...
reeds_offshore_wind_curve_capacity %>%
  ...
  left_join_error_no_match(L120.offshore_wind_CF_reeds, by = c("Wind_Resource_Region" , "Wind_Class" = "TRG")) %>%
...
reeds_offshore_wind_curve_grid_cost %>%
  ...
  mutate(grid.cost = fcr*cost/(CONV_YEAR_HOURS*CF*CONV_MWH_GJ)) %>%
  mutate(grid.cost = grid.cost*(gdp_deflator(1975,2013))) %>%
...
# Produce outputs
    ...
    L120.RsrcCurves_EJ_R_offshore_wind_USA %>%
      ...
      add_precursors("gcam-usa/reeds_regions_states", "energy/A23.globaltech_capital", "energy/A23.globaltech_OMfixed", "gcam-usa/A20.offshore_wind_class_depth", "energy/A20.offshore_wind_depth_cap_cost", "gcam-usa/reeds_offshore_wind_curve_capacity", "energy/reeds_offshore_wind_curve_grid_cost", "energy/reeds_offshore_wind_curve_CF_avg") ->
      L120.RsrcCurves_EJ_R_offshore_wind_USA
    ...
    L120.GridCost_offshore_wind_USA %>%
      ...
      add_precursors( "energy/reeds_offshore_wind_curve_grid_cost", "gcam-usa/reeds_regions_states", "energy/reeds_offshore_wind_curve_CF_avg") ->
      L120.GridCost_offshore_wind_USA
