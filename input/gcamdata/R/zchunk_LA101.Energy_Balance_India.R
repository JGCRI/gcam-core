#' module_gcam.india_LA101.Energy_Balance
#'
...
#'  \item{L101.india_state_EB_SecFuel_mtoe: Energy data by sector and fuel, GCAM sector and fuel, state, and year; energy units in mtoe, years from 1960-2011, includes all original data}
#' }
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{L101.india_state_EB_SecFuel_mtoe}, \code{L101.india_state_EB_EJ_state_S_F}.
#' @details See above
...
module_gcam.india_LA101.Energy_Balance <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "gcam-india/EB_FuelMap",
             FILE = "gcam-india/EB_SectorMap",
             FILE = "gcam-india/EB_SecFuel_mtoe",
             FILE = "gcam-india/fuel_conv"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("L101.india_state_EB_SecFuel_mtoe",
    ...
    EB_FuelMap                   <- get_data(all_data, "gcam-india/EB_FuelMap")
    EB_SectorMap                 <- get_data(all_data, "gcam-india/EB_SectorMap")
    EB_SecFuel_mtoe              <- get_data(all_data, "gcam-india/EB_SecFuel_mtoe")
    fuel_conv                    <- get_data(all_data, "gcam-india/fuel_conv")
    ...
    EB_SecFuel_mtoe %>%
      gather_years %>%
      left_join(EB_FuelMap, by = "GHG_Platform_fuels") %>%
      left_join(EB_SectorMap, by = "GHG_Platform_sectors") %>%
    ...
    mtoe_with_GCAM_names_intermediate %>%
      ...
      L101.india_state_EB_SecFuel_mtoe
    ...
    L101.india_state_EB_SecFuel_mtoe %>%
      ...
      add_legacy_name("L101.india_state_EB_SecFuel_mtoe") %>%
      add_precursors("gcam-india/EB_SecFuel_mtoe",
                     "gcam-india/EB_FuelMap",
                     "gcam-india/EB_SectorMap") ->
      L101.india_state_EB_SecFuel_mtoe
    ...
    L101.india_state_EB_EJ_state_S_F %>%
      ...
      add_precursors("gcam-india/EB_SecFuel_mtoe",
                     "gcam-india/EB_FuelMap",
                     "gcam-india/EB_SectorMap",
                     "gcam-india/fuel_conv") ->
      L101.india_state_EB_EJ_state_S_F

    return_data(L101.india_state_EB_SecFuel_mtoe, L101.india_state_EB_EJ_state_S_F)
  } else {
