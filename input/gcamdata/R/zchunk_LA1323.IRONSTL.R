...
#' module_energy_LA1323.IRONSTL
#'
#' Sets up input, output, and IO coefficients for IRONSTL and subtracts input energy from industry energy use
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{L1323.out_Mt_R_IRONSTL_Yh}, \code{L1323.IO_GJkg_R_IRONSTL_F_Yh}, \code{L1323.in_EJ_R_IRONSTL_F_Y}, \code{L1323.in_EJ_R_indenergy_F_Yh}. The corresponding file in the
#' original data system was \code{LA1323.IRONSTL.R} (energy level1).
#' @details This chunk generates input, output, and IO coefficients for the IRONSTL sector. It begins by downscaling Worrell regional data from 1994
...
#' are dealt with by moving their accounting to the IRONSTL sector.
...
module_energy_LA1323.IRONSTL <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "energy/steel_prod",
             FILE = "energy/IO_IRONSTL_scaled",
             "L1322.in_EJ_R_indenergy_F_Yh"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("L1323.out_Mt_R_IRONSTL_Yh",
    ...
    share <- value <- IRONSTL <- year <- value.y <- value.x <- NULL

    all_data <- list(...)[[1]]

    # Load required inputs
    All_steel <- get_data(all_data, "energy/steel_prod")
    IO_IRONSTL <- get_data(all_data, "energy/IO_IRONSTL_scaled")
    ...
    L1323.out_Mt_R_IRONSTL_Yh %>%
      ...
      left_join(IO_IRONSTL %>% rename(sector = sector.name,subsector = subsector.name), by = c("subsector","technology","year","region","GCAM_region_ID")) %>%
    ...
    L1322.in_EJ_R_indenergy_F_Yh %>%
      rename(raw = value) %>%
      left_join(L1323.in_EJ_R_IRONSTL_F_Y %>%
    ...
    L1323.out_Mt_R_IRONSTL_Yh %>%
      add_title("Historical IRONSTL outputs by region, fuel, and year") %>%
      add_units("Mt IRONSTL") %>%
      add_comments("Outputs are collecting from World steel association and then aggregating to GCAM regions") %>%
      add_legacy_name("L1323.out_Mt_R_IRONSTL_Yh") %>%
    ...
    L1323.IO_GJkg_R_IRONSTL_F_Yh %>%
      ...
      add_legacy_name("L1323.IO_GJkg_R_IRONSTL_F_Yh") %>%
      add_precursors( "energy/steel_prod", "energy/IO_IRONSTL_scaled") ->
      L1323.IO_GJkg_R_IRONSTL_F_Yh

    L1323.in_EJ_R_IRONSTL_F_Y %>%
      add_title("Historical input energy use for the IRONSTL sector") %>%
      add_units("Exajoules") %>%
      add_comments("Calculated by steel production and IO coefficients") %>%
      add_legacy_name("L1323.in_EJ_R_IRONSTL_F_Y") %>%
      add_precursors("energy/steel_prod","energy/IO_IRONSTL_scaled") ->
      L1323.in_EJ_R_IRONSTL_F_Y
    ...
    L1323.in_EJ_R_indenergy_F_Yh %>%
      ...
      add_units("Exajoules") %>%
      add_comments("Subtracted IRONSTL energy use from industrial energy use values in L1322.in_EJ_R_indenergy_F_Yh") %>%
    ...
    return_data(L1323.out_Mt_R_IRONSTL_Yh, L1323.IO_GJkg_R_IRONSTL_F_Yh, L1323.in_EJ_R_IRONSTL_F_Y, L1323.in_EJ_R_indenergy_F_Yh)
