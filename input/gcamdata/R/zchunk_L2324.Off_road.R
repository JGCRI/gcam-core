...
module_energy_L2324.Off_road <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    A324.sector <- get_data(all_data, "energy/A324.sector")
    A_regions <- get_data(all_data, "energy/A_regions")
    A324.subsector_interp <- get_data(all_data, "energy/A324.subsector_interp")
    A324.subsector_logit <- get_data(all_data, "energy/A324.subsector_logit")
    A324.subsector_shrwt <- get_data(all_data, "energy/A324.subsector_shrwt")
    A324.globaltech_coef <- get_data(all_data, "energy/A324.globaltech_coef")
    A324.globaltech_eff <- get_data(all_data, "energy/A324.globaltech_eff")
    A324.globaltech_cost <- get_data(all_data, "energy/A324.globaltech_cost")
	  A324.globaltech_retirement <- get_data(all_data, "energy/A324.globaltech_retirement")
	  A324.nonenergy_Cseq <- get_data(all_data, "energy/A324.nonenergy_Cseq")
    A324.globaltech_shrwt <- get_data(all_data, "energy/A324.globaltech_shrwt")
    A324.demand <- get_data(all_data, "energy/A324.demand")
