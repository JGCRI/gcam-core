...
module_gcamusa_LA1233.Process_UCS_data_ref <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    for(year_i in gcamusa.UCS_WATER_COEFFICIENTS_FIRST_HISTORICAL_YEAR:gcamusa.UCS_WATER_COEFFICIENTS_FINAL_HISTORICAL_YEAR){
      ...
      capacity_tech %>%
        left_join_error_no_match(capacity_tech_state %>%
                                   rename (Cap_MW_State = Cap_MW),
    ...
    cooling_share_future_complete_tech_FIRST_FUTURE_YEAR %>%
      bind_rows(cooling_share_future_complete_tech_FIRST_FUTURE_YEAR %>%
                  mutate(year = gcamusa.UCS_WATER_COEFFICIENTS_FINAL_FUTURE_YEAR)) ->
    ...
    cooling_share_historical %>%
      bind_rows(cooling_share_historical %>%
                  filter(year == gcamusa.UCS_WATER_COEFFICIENTS_FINAL_CALIBRATION_YEAR) %>%
                  mutate(year = gcamusa.UCS_WATER_COEFFICIENTS_FINAL_HISTORICAL_YEAR)) %>%
    ...
    complete_tech_cooling_share_Edited %>%
      ...
      gather(key = `Cooling Technology`, value = Cap_MW,
             -Fuel, -technology, -State, -plant_type, -`Reported Water Source (Type)`, -year, -NEMS) %>%
      group_by(plant_type, Fuel, technology, State, NEMS, year) %>%
      summarise(sum_byTech_Cap_MW = sum(Cap_MW, na.rm = TRUE)) %>%
      ungroup() ->
