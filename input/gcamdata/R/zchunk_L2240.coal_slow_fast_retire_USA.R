...
#' the generated outputs: \code{L2240.coal_conv_pul_delete_USA}, \code{L2240.StubTechProd_elecS_coal_USA}, \code{L2240.StubTechShrwt_elecS_coal_USA},
...
#' \code{L2240.GlobalTechEff_elec_coalret_USA}, \code{L2240.GlobalTechProfitShutdown_elec_coalret_USA}, \code{L2240.ghg_emissions_coalret_USA}.
...
module_gcam.usa_L2240.coal_slow_fast_retire_USA <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Allow new investment in coal_base_conv pul and coal_int_conv pul starting in 2035
    # Set shareweights for these technologies to 0 before FIRST_NEW_COAL_YEAR, 1 thereafter
    A23.elec_tech_associations_coal_retire %>%
      select("supplysector" = "Electric.sector", "subsector", "stub.technology" = "technology") %>%
      anti_join(A23.coal_conv_pul_delete, by = c("supplysector", "subsector", "stub.technology")) %>%
      write_to_all_states(c("region", "supplysector", "subsector", "stub.technology")) %>%
      repeat_add_columns(tibble(year = FUTURE_YEARS)) %>%
      mutate(share.weight = if_else(year < gcamusa.FIRST_NEW_COAL_YEAR, 0, 1)) ->
      L2240.StubTechShrwt_elecS_coal_USA
    ...
    L2240.StubTechShrwt_elecS_coal_USA %>%
      add_title("Revised share weights for state-level coal technologies which are not deleted") %>%
      add_units("Unitless") %>%
      add_comments("Adjusting share weights for technologies that will be allocated to fast retire and slow retire technologies") %>%
      add_comments("Share weights set to zero in historical periods and near-future") %>%
