...
module_energy_LA1323.iron_steel <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    DRI_stats %>%
      ...
      mutate(DRI_consumption=`DRI production`- `DRI exports` + `DRI imports`)%>%
      mutate(DRI_consumption=ifelse(DRI_consumption<0,0,DRI_consumption))%>%
    ...
    All_steel %>%
      ...
      mutate(`EAF with DRI`=ifelse(`EAF with scrap`<=0,EAF,`EAF with DRI`))%>%
      mutate(`EAF with scrap`=ifelse(`EAF with scrap`<0,0,`EAF with scrap`))%>%
      select(-EAF)%>%
    ...
    All_steel %>%
      ...
      All_steel %>%
        ...
        mutate(`EAF with scrap`=ifelse(`EAF with scrap`==0 & year == MODEL_FINAL_BASE_YEAR,(BLASTFUR+`EAF with scrap`+`EAF with DRI`)*0.005,`EAF with scrap`))%>%
        mutate(`EAF with DRI`=ifelse(`EAF with DRI`==0 & year == MODEL_FINAL_BASE_YEAR,(BLASTFUR+`EAF with scrap`+`EAF with DRI`)*0.005,`EAF with DRI`))%>%
        #calculate the percentage of BLASTFUR, EAF with scrap, and EAF with DRI across regions and years
        mutate(BLASTFUR_pct=BLASTFUR/(BLASTFUR+`EAF with scrap`+`EAF with DRI`),
