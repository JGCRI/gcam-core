...
#' module_energy_LB1092.iron_steel_GrossTrade
#'
...
module_energy_LB1092.iron_steel_GrossTrade <- function(command, ...){
  if(command == driver.DECLARE_INPUTS) {
    ...
    WSA_steel_all_1970_2018 <- rbind(WSA_steel_prod_cons_1970_2018,WSA_steel_trade_1970_2018) %>%
      ...
      mutate(value=value/1000) %>% #Unit: to Mt
      mutate(value=replace_na(value,0)) #replace missing data with zero
    ...
    LB1092.Tradebalance_iron_steel_Mt_R_Y %>%
      ...
      mutate(scaling_factor=(consumption-production)/(imports-exports))%>%
      #re-evaluate the exports and imports by multiplying with the scaling factor
      mutate(exports_reval=scaling_factor*exports,imports_reval=scaling_factor*imports)%>%
      #for regions with negative scaling factors use the WSA imports and exports
      mutate(exports_reval=ifelse(exports_reval<0,exports,exports_reval),imports_reval=ifelse(imports_reval<0,imports,imports_reval))%>%
      #estimate the % increase in imports and exports after scaling
      mutate(diff_exports=ifelse(exports==0,0,((exports_reval-exports)/exports)*100)) %>%
      mutate(diff_imports=ifelse(imports==0,0,((imports_reval-imports)/imports)*100)) %>%
      ...
      mutate(exports_reval=ifelse(diff_exports>=10 | diff_exports <=-10,exports,exports_reval),imports_reval=ifelse(diff_imports >=10 |diff_imports <=-10,imports,imports_reval))%>%
      ...
      mutate(exports_reval=ifelse(production-exports_reval<0,0,exports_reval),imports_reval=ifelse(production-exports_reval<0,0,imports_reval))%>%
      mutate(domestic_supply=production-exports_reval)%>%
      #estimate steel consumption using the final scaled imports and exports
      mutate(consumption_reval=production-exports_reval+imports_reval) %>%
    ...
    Global_scaling_factors <- LB1092.Tradebalance_iron_steel_Mt_R_Y %>%
      ...
      mutate(imports_export_scale=replace_na(imports_export_scale,1))%>%
    ...
    LB1092.Tradebalance_iron_steel_Mt_R_Y %>%
      ...
      mutate(intra_exports=value*percent_trade/100) %>%
      mutate(intra_exports=ifelse(is.na(intra_exports),0,intra_exports))%>%
    ...
    LB1092.Tradebalance_iron_steel_Mt_R_Y %>%
      ...
      mutate(value=ifelse(metric %in% c("domestic_supply"),value+intra_exports,value))%>%
