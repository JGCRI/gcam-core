...
#'
module_socioeconomics_L280.GDP_macro <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    savings.rate.hist %>% arrange(GCAM_region_ID, period) %>%
      ...
      mutate(savings.rate.lag = lag(savings.rate)) %>%
      ungroup() -> savings.rate.hist
    ...
    national.accounts.FutureYrs %>% mutate(gdp.cap = gdp/pop) %>%
      mutate( gdp.cap.gr=0, savings.rate = 0, savings.rate.lag = 0, savings.rate.fit = 0 ) %>%
    ...
    savings.rate.all %>% arrange(GCAM_region_ID, period) %>%
      ...
      mutate(savings.rate.fit2 = savings.rate) %>%
