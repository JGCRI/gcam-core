...
#' module_energy_L2323.IRONSTL
#'
#' Compute a variety of final energy keyword, sector, share weight, and technology information for IRONSTL-related GCAM inputs.
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{L2323.SectorLogitTables[[ curr_table ]]$data}, \code{L2323.Supplysector_IRONSTL}, \code{L2323.FinalEnergyKeyword_IRONSTL},
#' \code{L2323.SubsectorLogitTables[[ curr_table ]]$data}, \code{L2323.SubsectorLogit_IRONSTL}, \code{L2323.SubsectorShrwtFllt_IRONSTL},
#' \code{L2323.SubsectorInterp_IRONSTL}, \code{L2323.StubTech_IRONSTL}, \code{L2323.GlobalTechShrwt_IRONSTL}, \code{L2323.GlobalTechCoef_IRONSTL},
#' \code{L2323.GlobalTechCost_IRONSTL}, \code{L2323.GlobalTechCapture_IRONSTL}, \code{L2323.StubTechProd_IRONSTL}, \code{L2323.StubTechCalInput_IRONSTL},
#' \code{L2323.StubTechCoef_IRONSTL}, \code{L2323.PerCapitaBased_IRONSTL}, \code{L2323.BaseService_IRONSTL}, \code{L2323.PriceElasticity_IRONSTL},
#' \code{object}. The corresponding file in the
#' original data system was \code{L2323.IRONSTL.R} (energy level2).
#' @details The chunk provides final energy keyword, supplysector/subsector information, supplysector/subsector interpolation information, global technology share weight, global technology efficiency, global technology coefficients, global technology cost, price elasticity, stub technology information, stub technology interpolation information, stub technology calibrated inputs, and etc for IRONSTL sector.
...
module_energy_L2323.IRONSTL <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    A323.sector <- get_data(all_data, "energy/A323.sector")
    A323.subsector_interp <- get_data(all_data, "energy/A323.subsector_interp")
    A323.subsector_logit <- get_data(all_data, "energy/A323.subsector_logit")
    A323.subsector_shrwt <- get_data(all_data, "energy/A323.subsector_shrwt")
    A323.globaltech_coef <- get_data(all_data, "energy/A323.globaltech_coef")
    A323.globaltech_cost <- get_data(all_data, "energy/A323.globaltech_cost")
    A323.globaltech_shrwt <- get_data(all_data, "energy/A323.globaltech_shrwt")
    A323.globaltech_co2capture <- get_data(all_data, "energy/A323.globaltech_co2capture")
    A323.globaltech_retirement <- get_data(all_data, "energy/A323.globaltech_retirement")
    A323.demand <- get_data(all_data, "energy/A323.demand")
    L1323.out_Mt_R_IRONSTL_Yh <- get_data(all_data, "L1323.out_Mt_R_IRONSTL_Yh")
    L1323.IO_GJkg_R_IRONSTL_F_Yh <- get_data(all_data, "L1323.IO_GJkg_R_IRONSTL_F_Yh")
    L1323.in_EJ_R_IRONSTL_F_Y <- get_data(all_data, "L1323.in_EJ_R_IRONSTL_F_Y")
    # ===================================================
    ...
    # L2323.Supplysector_IRONSTL: Supply sector information for IRONSTL sector
    A323.sector %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["Supplysector"]], LOGIT_TYPE_COLNAME), GCAM_region_names) ->
      L2323.Supplysector_IRONSTL

    # L2323.FinalEnergyKeyword_IRONSTL: Supply sector keywords for IRONSTL sector
    A323.sector %>%
    ...
    # L2323.SubsectorLogit_IRONSTL: Subsector logit exponents of IRONSTL sector
    A323.subsector_logit %>%
      write_to_all_regions(c(LEVEL2_DATA_NAMES[["SubsectorLogit"]], LOGIT_TYPE_COLNAME), GCAM_region_names) ->
      L2323.SubsectorLogit_IRONSTL

    # and L2323.SubsectorShrwtFllt_IRONSTL: Subsector shareweights of IRONSTL sector
    A323.subsector_shrwt %>%
      filter(!is.na(year.fillout)) %>%
      write_to_all_regions(LEVEL2_DATA_NAMES[["SubsectorShrwtFllt"]], GCAM_region_names) ->
      L2323.SubsectorShrwtFllt_IRONSTL

    # L2323.SubsectorInterp_IRONSTL: Subsector shareweight interpolation of IRONSTL sector
    ...
    # L2323.StubTech_IRONSTL: Identification of stub technologies of IRONSTL
    ...
    # L2323.GlobalTechShrwt_IRONSTL: Shareweights of global IRONSTL technologies
    A323.globaltech_shrwt %>%
      gather_years %>%
    ...
    # L2323.GlobalTechCoef_IRONSTL: Energy inputs and coefficients of IRONSTL technologies
    ...
    # L2323.GlobalTechCapture_IRONSTL: CO2 capture fractions from global IRONSTL production technologies with CCS
    ...
    # L2323.GlobalTechCost_IRONSTL: Non-energy costs of global IRONSTL manufacturing technologies
    ...
    # L2323.StubTechProd_IRONSTL: calibrated IRONSTL production
    calibrated_techs %>%
    ...
    # L2323.StubTechCoef_IRONSTL: region-specific coefficients of IRONSTL production technologies
    ...
    # L2323.StubTechCalInput_IRONSTL: calibrated IRONSTL production
    # calibrated_techs %>%
      # select(sector, fuel, supplysector, subsector, technology, minicam.energy.input) %>%
      # distinct ->
      # calibrated_techs_export # temporary tibble
    ...
    # L2323.PerCapitaBased_IRONSTL: per-capita based flag for IRONSTL exports final demand
    A323.demand %>%
      write_to_all_regions(LEVEL2_DATA_NAMES[["PerCapitaBased"]], GCAM_region_names)  ->
      L2323.PerCapitaBased_IRONSTL


    # L2323.BaseService_IRONSTL: base-year service output of IRONSTL
    L2323.StubTechProd_IRONSTL %>%
    ...
    # L2323.PriceElasticity_IRONSTL: price elasticity
    A323.demand %>%
    ...
    # Produce outputs
    L2323.Supplysector_IRONSTL %>%
      add_title("Supply sector information for IRONSTL sector") %>%
      add_units("NA") %>%
      add_comments("For IRONSTL sector, the supply sector information (output.unit, input.unit, price.unit, logit.year.fillout, logit.exponent) from A323.sector is expended into all GCAM regions") %>%
      add_legacy_name("L2323.Supplysector_IRONSTL") %>%
      add_precursors("energy/A323.sector", "common/GCAM_region_names") ->
      L2323.Supplysector_IRONSTL

    L2323.FinalEnergyKeyword_IRONSTL %>%
      add_title("Supply sector keywords for IRONSTL sector") %>%
      add_units("NA") %>%
      add_comments("For IRONSTL sector, the supply sector final energy keywords from A323.sector are expended into all GCAM regions") %>%
      add_legacy_name("L2323.FinalEnergyKeyword_IRONSTL") %>%
      add_precursors("energy/A323.sector", "common/GCAM_region_names") ->
      L2323.FinalEnergyKeyword_IRONSTL

    L2323.SubsectorLogit_IRONSTL %>%
      add_title("Subsector logit exponents of IRONSTL sector") %>%
      add_units("Unitless") %>%
      add_comments("For IRONSTL sector, the subsector logit exponents from A323.subsector_logit are expanded into all GCAM regions") %>%
      add_legacy_name("L2323.SubsectorLogit_IRONSTL") %>%
      add_precursors("energy/A323.subsector_logit", "common/GCAM_region_names") ->
      L2323.SubsectorLogit_IRONSTL

    L2323.SubsectorShrwtFllt_IRONSTL %>%
      add_title("Subsector shareweights of IRONSTL sector") %>%
      add_units("unitless") %>%
      add_comments("For IRONSTL sector, the subsector shareweights from A323.subsector_shrwt are expanded into all GCAM regions") %>%
      add_legacy_name("L2323.SubsectorShrwtFllt_IRONSTL") %>%
      add_precursors("energy/A323.subsector_shrwt", "common/GCAM_region_names") ->
      L2323.SubsectorShrwtFllt_IRONSTL

    L2323.SubsectorInterp_IRONSTL %>%
      add_title("Subsector shareweight interpolation of IRONSTL sector") %>%
      add_units("NA") %>%
      add_comments("For IRONSTL sector, the subsector shareweight interpolation function infromation from A323.subsector_interp is expanded into all GCAM regions") %>%
      add_legacy_name("L2323.SubsectorInterp_IRONSTL") %>%
      add_precursors("energy/A323.subsector_interp", "common/GCAM_region_names") ->
      L2323.SubsectorInterp_IRONSTL

    L2323.StubTech_IRONSTL %>%
      add_title("Identification of stub technologies of IRONSTL") %>%
      add_units("NA") %>%
      add_comments("For IRONSTL sector, the stub technologies from A323.globaltech_shrwt are expanded into all GCAM regions") %>%
      add_legacy_name("L2323.StubTech_IRONSTL") %>%
      add_precursors("energy/A323.globaltech_shrwt", "common/GCAM_region_names") ->
      L2323.StubTech_IRONSTL

    L2323.GlobalTechShrwt_IRONSTL %>%
      add_title("Shareweights of global IRONSTL technologies") %>%
      add_units("Unitless") %>%
      add_comments("For IRONSTL sector, the share weights from A323.globaltech_shrwt are interpolated into all base years and future years") %>%
      add_legacy_name("L2323.GlobalTechShrwt_IRONSTL") %>%
      add_precursors("energy/A323.globaltech_shrwt") ->
      L2323.GlobalTechShrwt_IRONSTL

    L2323.GlobalTechCoef_IRONSTL %>%
      add_title("Energy inputs and coefficients of IRONSTL technologies") %>%
      add_units("scrap input is unitless (Mt scrap per Mt IRONSTL); all others are GJ per kg (EJ of energy per Mt of IRONSTL)") %>%
      add_comments("For IRONSTL sector, the energy use coefficients from A323.globaltech_coef are interpolated into all model years") %>%
      add_legacy_name("L2323.GlobalTechCoef_IRONSTL") %>%
      add_precursors("energy/A323.globaltech_coef") ->
      L2323.GlobalTechCoef_IRONSTL

    L2323.GlobalTechCost_IRONSTL %>%
      add_title("Non-energy costs of global IRONSTL manufacturing technologies") %>%
      add_units("1975$/kg for supplysector IRONSTL; 1975$/GJ for supplysector process heat IRONSTL") %>%
      add_comments("For IRONSTL sector, the Non-energy costs of global IRONSTL manufacturing technologies are calculated then adjusted with CCS to include CO2 capture costs") %>%
      add_legacy_name("L2323.GlobalTechCost_IRONSTL") %>%
    ...
    L2323.GlobalTechCapture_IRONSTL %>%
      ...
      add_units("Unitless") %>%
      add_comments("For IRONSTL sector, the remove fractions from A323.globaltech_co2capture are interpolated into all model years") %>%
      add_legacy_name("L2323.GlobalTechCapture_IRONSTL") %>%
    ...
    L2323.StubTechProd_IRONSTL %>%
      add_title("calibrated IRONSTL production") %>%
      add_units("Mt") %>%
      add_comments("Values are calculated using L1323.out_Mt_R_IRONSTL_Yh then added GCAM region information and supplysector, subsector, and technology information") %>%
      add_legacy_name("L2323.StubTechProd_IRONSTL") %>%
      add_precursors("energy/calibrated_techs", "L1323.out_Mt_R_IRONSTL_Yh", "common/GCAM_region_names") ->
      L2323.StubTechProd_IRONSTL

    # L2323.StubTechCalInput_IRONSTL %>%
      # add_title("calibrated IRONSTL production") %>%
      # add_units("EJ") %>%
      # add_comments("Values are calculated using L1323.in_EJ_R_IRONSTL_F_Y then added GCAM region information and supplysector, subsector, technology, and input information") %>%
      # add_legacy_name("L2323.StubTechCalInput_IRONSTL") %>%
      # add_precursors("energy/calibrated_techs", "L1323.in_EJ_R_IRONSTL_F_Y", "common/GCAM_region_names") ->
      # L2323.StubTechCalInput_IRONSTL

    L2323.StubTechCoef_IRONSTL %>%
      add_title("region-specific coefficients of IRONSTL production technologies") %>%
      add_units("scrap input is unitless (Mt scrap per Mt IRONSTL); all others are GJ per kg (EJ of energy per Mt of IRONSTL)") %>%
      add_comments("Coefficients are calculated using L1323.IO_GJkg_R_IRONSTL_F_Yh") %>%
      add_legacy_name("L2323.StubTechCoef_IRONSTL") %>%
      add_precursors("energy/calibrated_techs", "L1323.IO_GJkg_R_IRONSTL_F_Yh", "common/GCAM_region_names") ->
      L2323.StubTechCoef_IRONSTL

    L2323.PerCapitaBased_IRONSTL %>%
      add_title("per-capita based flag for IRONSTL exports final demand") %>%
      add_units("NA") %>%
      add_comments("Per-capita based flags for IRONSTL from A323.demand are expanded into all GCAM regions") %>%
      add_legacy_name("L2323.PerCapitaBased_IRONSTL") %>%
      add_precursors("energy/A323.demand", "common/GCAM_region_names") ->
      L2323.PerCapitaBased_IRONSTL

    L2323.BaseService_IRONSTL %>%
      add_title("base-year service output of IRONSTL") %>%
      add_units("Mt") %>%
      add_comments("Transformed from L2323.StubTechProd_IRONSTL by adding energy.final.demand from A323.demand") %>%
      add_legacy_name("L2323.BaseService_IRONSTL") %>%
      add_precursors("energy/A323.demand", "energy/calibrated_techs", "L1323.out_Mt_R_IRONSTL_Yh", "common/GCAM_region_names") ->
      L2323.BaseService_IRONSTL

    L2323.PriceElasticity_IRONSTL %>%
      add_title("price elasticity for IRONSTL") %>%
      add_units("Unitless") %>%
      add_comments("The elasticity values from A323.demand are expanded into all GCAM_regions") %>%
      add_legacy_name("L2323.PriceElasticity_IRONSTL") %>%
      add_precursors("energy/A323.demand", "common/GCAM_region_names") ->
      L2323.PriceElasticity_IRONSTL

      return_data(L2323.Supplysector_IRONSTL, L2323.FinalEnergyKeyword_IRONSTL, L2323.SubsectorLogit_IRONSTL,
                  L2323.SubsectorShrwtFllt_IRONSTL, L2323.SubsectorInterp_IRONSTL,
                  L2323.StubTech_IRONSTL, L2323.GlobalTechShrwt_IRONSTL,
                  L2323.GlobalTechCoef_IRONSTL, L2323.GlobalTechCost_IRONSTL, L2323.GlobalTechCapture_IRONSTL, L2323.GlobalTechShutdown_en,
                  L2323.GlobalTechSCurve_en, L2323.GlobalTechLifetime_en, L2323.GlobalTechProfitShutdown_en,
                  L2323.StubTechProd_IRONSTL, L2323.StubTechCoef_IRONSTL,
                  L2323.PerCapitaBased_IRONSTL, L2323.BaseService_IRONSTL,
                  L2323.PriceElasticity_IRONSTL)
				#L2323.StubTechCalInput_IRONSTL
  } else {
