...
#' the generated outputs: \code{HDDCDD_constdd_no_GCM.xml}.
module_energy_HDDCDD_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c("L244.HDDCDD_constdd_no_GCM"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c(XML = "HDDCDD_constdd_no_GCM.xml"))
    ...
    return_data(HDDCDD_constdd_no_GCM.xml)
  } else {
