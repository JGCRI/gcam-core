...
module_energy_L1326.aluminum <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    aluminum_prod_country <- get_data(all_data, "energy/aluminum_prod_USGS") %>% bind_rows(get_data(all_data, "energy/aluminum_prod_CEDS"))
    aluminum_prod_region <- get_data(all_data, "energy/aluminum_prod_region_IAI")
    aluminum_energy_region_IAI <- get_data(all_data, "energy/aluminum_energy_region_IAI")
    alumina_energy_region_IAI <- get_data(all_data, "energy/alumina_energy_region_IAI")
    IAI_ctry_region <- get_data(all_data, "energy/mappings/IAI_ctry_region")
    ...
    if (Prod_Ctry_max_year < FINAL_HISTORICAL_YEAR) {
    ...
    if (Prod_R_max_year < FINAL_HISTORICAL_YEAR) {
    ...
    # First downscale IAI production and energy data to countries, using country-level USGS data
    ...
    L1326.in_EJ_R_aluminum_Yh %>%
      ...
      add_precursors("energy/A_regions", "energy/aluminum_prod_USGS", "energy/aluminum_energy_region_IAI", "energy/alumina_energy_region_IAI",
                     "energy/mappings/IAI_ctry_region", "common/iso_GCAM_regID", "energy/mappings/enduse_fuel_aggregation") ->
      L1326.in_EJ_R_aluminum_Yh
