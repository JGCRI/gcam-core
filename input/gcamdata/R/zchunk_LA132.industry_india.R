...
module_gcamindia_LA132.industry <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L101.india_state_EB_EJ_state_S_F %>%
      ...
      L132.india_state_in_EJ_ind_elecgas_adj
    ...
    L132.india_state_in_pct_ind_F %>%
      ...
      left_join_error_no_match(filter(L132.in_EJ_R_indenergy_F_Yh, GCAM_region_ID == gcam.INDIA_CODE),
    ...
    L132.india_state_pct_indfeed_F %>%
      ...
      # ^^ prepare for smooth join
      left_join_error_no_match(filter(L1322.in_EJ_R_indfeed_F_Yh, GCAM_region_ID == gcam.INDIA_CODE),
