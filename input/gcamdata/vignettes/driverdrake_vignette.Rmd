...
On Windows we may run into the MAX_PATH file path limit after the package is installed. If you get the following error, make sure you load the package with `devtools::load_all()` or reduce the file path by shortening the directory name or moving the workspace to have a shorter path length.
```
