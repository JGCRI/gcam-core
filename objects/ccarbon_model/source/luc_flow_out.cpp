...
void LUCFlowOut::transfer( const double aValue, const EnvironmentalInfo* aEnvInfo,
	...
	//! notice, the aBoxType has no usage here. We are not transfer to same box type
...
void LUCFlowOut::completeInit( const std::map< const std::string, ICarbonContainer* > aNamesToBoxes,
                               int aKey ){
    //Set out target to be the appropriate container in the summer box.
    SummerBox* summer = SummerBox::getInstance();
    mTarget = SummerBox::getInstance()->getContainer( aKey );
}
