...
public:
	...
	/*!
     * \brief Returns the current NPP/area unit.
     * \return the current NPP/area unit.
     */
    virtual double getNPPOverAreaRatio( const int aYear ) = 0;

	/*!
	 * \brief deduct the atmosphere carbon value with total NPP value
	 * \deduct the number of current carbon value with the NPP value from the atmosphere box
	 * \param  double aDeductNPPValue, a total NPP value calculated dynamically
	 * \param  int aYear, the year of of carbon stock to deduct from
	 */

	virtual void deductFromStock( const double aDeductNPPValue, const int aYear ) = 0;
