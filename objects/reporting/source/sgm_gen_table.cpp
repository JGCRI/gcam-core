...
void SGMGenTable::updateTradeConsumer( const TradeConsumer* tradeConsumer,
									   const int aPeriod ) 
{
	// net energy trade
	if(	mName == "ETRADE" )	{
		...
		for( unsigned int i = 0; i < tradeConsumer->input.size(); ++i ){
			...
			const double tradedEmissions = tradeConsumer->input[ i ]->getDemandPhysical( mCurrentRegionName ) 
		...
		if( (prodTech->categoryName == "PassTransport") && (prodTech->mOutputs[ aPeriod ] != 0) ){
			...
			// for all other tables that require inputs
				...
				for( unsigned int i=0; i<prodTech->input.size(); i++ ){
					...
					double fuelConsumption = prodTech->input[ i ]->getDemandCurrency() * prodTech->input[ i ]->getConversionFactor( mCurrentRegionName );
