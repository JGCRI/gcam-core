...
#if !defined( __RESIDUEBIOMASSPRODUCTION_H )
#define __RESIDUEBIOMASSPRODUCTION_H    // prevent multiple includes
...
// class: ResidueBiomassProduction *********************************************
 ...
 *
 *   <b>XML specification for ResidueBiomassProduction</b>
 *   - XML name: \c residue-biomass
 *   - Contained by: Technology
 *   - Parsing inherited from class: None.
 *   - Attributes: none
 *   - Elements:
 *   - \c curve-exponent ResidueBiomassProduction::mCostCurve.get/setCurveExponent()
 *   - \c eros-ctrl ResidueBiomassProduction::mErosCtrl
 *   - \c harvest-index ResidueBiomassProduction::mHarvestIndex
 *   - \c mass-conversion ResidueBiomassProduction::mMassConversion
 *   - \c mass-to-energy ResidueBiomassProduction::mMassToEnergy
 *   - \c mid-price ResidueBiomassProduction::mCostCurve.get/setMidprice()
 *
 * \author Kevin Walker
 * \date $ Date $
 * \version $ Revision $
 */
class ResidueBiomassProduction : public IOutput
{
public :
    ...
    */
   ResidueBiomassProduction( const std::string& sectorName = std::string() )
      : parent(),
        mPhysicalOutputs(),
        mName(sectorName),
        mCachedCO2Coef(),
        ...
        mCostCurve() {}
   //! \pre Assigned(other)
   ResidueBiomassProduction( const ResidueBiomassProduction& other )
      : parent(),
   ...
   //! Destructor
   virtual ~ResidueBiomassProduction(void) {}
    ...
    */
   ResidueBiomassProduction& operator = ( const ResidueBiomassProduction& other )
   {
      if ( &other != this )
      {
         mPhysicalOutputs = other.mPhysicalOutputs;
   ...
   {
      return new ResidueBiomassProduction( *this );
   }
...
// end of residue_biomass_production.h *************************************
