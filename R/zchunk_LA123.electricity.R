...
module_energy_LA123.electricity <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L123.out_EJ_R_elec_F_Yh %>%
      ...
      select(-outputs,-input) -> L123.eff_R_elec_F_Yh
    ...
    L123.eff_R_elec_F_Yh %>%
      ...
      mutate(value =if_else(value==Inf,DEFAULT_ELECTRIC_EFFICIENCY,value)) -> L123.eff_R_elec_F_Yh
    ...
    L123.in_EJ_R_elec_F_Yh %>%
      ...
      select(-input,-value) -> Output_efficiency_based
    ...
    L123.out_EJ_R_elec_F_Yh %>%
      ...
      select(-outputs_new) -> L123.out_EJ_R_elec_F_Yh
    ...
    L123.out_EJ_R_indchp_F_Yh %>%
      ...
      select(-value,-elec_ratio) -> L123.in_EJ_R_indchp_F_Yh

   # Rename necessary outputs to value for long test flag
   L123.out_EJ_R_elec_F_Yh %>%
       mutate(value=outputs)%>%select(-outputs)->L123.out_EJ_R_elec_F_Yh

   L123.in_EJ_R_elec_F_Yh %>%
       mutate(value=input)%>%select(-input)->L123.in_EJ_R_elec_F_Yh

   L123.in_EJ_R_indchp_F_Yh %>%
      mutate(value=outputs_ratio)%>%select(-outputs_ratio)->L123.in_EJ_R_indchp_F_Yh
