...
module_water_L210.water.demand.primary <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L110.water_demand_primary_R_S_W_m3_GJ <- get_data(all_data, "L110.water_demand_primary_R_S_W_m3_GJ")
      ...
      A21.globaltech_coef %>%
        select(supplysector,subsector,technology) -> A21.tmp1
       A22.globaltech_coef %>% filter( grepl('nuclear', supplysector)) %>%
         select(supplysector,subsector,technology) -> A22.tmp
       bind_rows(A21.tmp1,A22.tmp) -> A21.A22.tmp
    ...
    results %>%
      ...
      add_comments("and then avoid double acounting unconventional oil in the regional oil sector, and then set the (water demand) coefficient
                   for each year, region, supplysector, subsector, technology, minicam.energy.input and market") %>%
