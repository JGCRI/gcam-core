...
#'
#' Downscale the countries that separated into multiple modern countries (e.g. USSR)
#'
#' @param data data
#' @param country_name country name
#' @param dissolution_year dissolution year
...
#' @export
downscale_FAO_country <- function(df, country_name, dissolution_year,
                                  item_name = "item", element_name = "element",
                                  years = AGLU_HISTORICAL_YEARS) {
  ...
  # data_new[X_ctry_years] <- data_new[[X_dissolution_year]] * data_ratio[match(vecpaste(data_new[c(item_name,
  #                                                                                                  element_name)]), vecpaste(data_ratio[c(item_name, element_name)])), X_ctry_years]
  #  data_new[X_ctry_years][is.na(data_new[X_ctry_years])] <- 0

  df %>%
    filter(year %in% c(ctry_years, dissolution_year)) %>%
    group_by_(item_name, element_name, "year") %>%
    summarise(value = sum(value)) %>%
    mutate(ratio = value / nth(value, which(year == dissolution_year))) %>%
    select(-value) ->
    data_ratio

  df %>%
    # data_new <- subset(data, countries != country_name)
    filter(countries != country_name,
           year %in% c(ctry_years, dissolution_year)) %>%
    # data_new[X_ctry_years] <- data_new[[X_dissolution_year]] * data_ratio[match(vecpaste(data_new[c(item_name,
    left_join(data_ratio, by = c(item_name, element_name, "year")) %>%
    group_by_(item_name, element_name, "iso") %>%
    mutate(value = nth(value, which(year == dissolution_year)) * ratio) %>%
    # data_new[X_ctry_years][is.na(data_new[X_ctry_years])] <- 0
  ...
  # test <- test[c(2, 3, 4, 5, 1, 6, 8:58, 7)]
  # names(test) <- names(old)
  #
  # test$element <- gsub(pattern = "_[A-Z]*$", "", test$element)
  # test$element <- gsub(pattern = "^FAO_", "", test$element)
  #
  # test <- arrange(test, countries, country.codes, item,
