...
#' the generated outputs: \code{L2240.coal_conv_pul_delete_USA}, \code{L2240.StubTechProd_elec_coalret_USA}, \code{L2240.StubTechEff_elec_coalret_USA},
#' \code{L2240.StubTechSCurve_elec_coalret_USA}, \code{L2240.StubTechMarket_elec_coalret_USA}, \code{L2240.GlobalTechShrwt_elec_coalret_USA},
#' \code{L2240.GlobalTechCapFac_elec_coalret_USA}, \code{L2240.GlobalTechCapital_elec_coalret_USA}, \code{L2240.GlobalTechOMfixed_elec_coalret_USA},
#' \code{L2240.GlobalTechOMvar_elec_coalret_USA}, \code{L2240.GlobalTechEff_elec_coalret_USA}, \code{L2240.GlobalTechProfitShutdown_elec_coalret_USA},
#' \code{L2240.ghg_emissions_coalret_USA}, \code{L2240.StubTechShrwt_coal_retire_elec_USA}.
...
module_gcam.usa_L2240.coal_slow_fast_retire_USA <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Delete old coal_base_conv pul stub-technology in the states
    ...
    L2240.elec_USA_coalret_base %>%
      ...
      select(LEVEL2_DATA_NAMES[["StubTechYr"]], calOutputValue, share.weight.year, subs.share.weight, share.weight) ->
      L2240.StubTechProd_elec_coalret_USA
    ...
    # L2240.ghg_emissions_coalret_USA: Non-CO2 emissions coefficients for coal fast and slow retire technologies
    A23.elec_tech_associations_coal_retire %>%
      repeat_add_columns(tibble(region = gcamusa.STATES)) %>%
      left_join(L2236.elecS_ghg_emissions_USA,
                by = c("region", "Electric.sector" = "supplysector", "subsector", "technology" = "stub.technology")) %>%
      rename(supplysector = Electric.sector, stub.technology = Electric.sector.technology) %>%
      left_join_error_no_match(fraction_fast_retire_generation, by = c("region")) %>%
      mutate(input.emissions = replace(input.emissions, grepl("fast_retire", stub.technology),
                                       input.emissions[grepl("fast_retire", stub.technology)] * fast.retire[grepl("fast_retire", stub.technology)]),
             input.emissions = replace(input.emissions, grepl("slow_retire", stub.technology),
                                       input.emissions[grepl("slow_retire", stub.technology)] * (1 - fast.retire[grepl("slow_retire", stub.technology)]))) %>%
    ...
    L2240.ghg_emissions_coalret_USA %>%
      add_title("Non-CO2 emissions coefficients for coal fast and slow retire technologies by load segment and state") %>%
      add_units("Unitless") %>%
      add_comments("Conventional coal electricity Non-CO2 emissions are allocated to fast retire and slow retire technologies") %>%
      add_legacy_name("L2240.ghg_emissions_coalret_USA") %>%
      add_precursors("gcam-usa/A23.elec_tech_associations_coal_retire",
                     "gcam-usa/fraction_fast_retire_generation",
                     "L2236.elecS_ghg_emissions_USA") ->
      L2240.ghg_emissions_coalret_USA
    ...
    L2240.StubTechShrwt_coal_retire_elec_USA %>%
      add_title("Shareweights of conventional U.S. coal electricity by load segment and state in future years") %>%
      add_units("Unitless") %>%
      add_comments("Set zero shareweight for conventional coal electricity technologies in all U.S. states and future years that represents no new coal") %>%
      add_legacy_name("L2240.StubTechShrwt_coal_retire_elec_USA") %>%
      add_precursors("gcam-usa/A23.elec_tech_associations_coal_retire") ->
      L2240.StubTechShrwt_coal_retire_elec_USA
