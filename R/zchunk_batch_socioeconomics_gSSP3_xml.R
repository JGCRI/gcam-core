#' module_socio_batch_gSSP3.xml
#'
#' Construct XML data structure for \code{socioeconomics_gSSP3.xml}.
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{socioeconomics_gSSP3.xml}. The corresponding file in the
#' original data system was \code{batch_socioeconomics_gSSP3.xml} (socio XML).
module_socio_batch_gSSP3.xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c("L201.Pop_gSSP3",
             "L201.BaseGDP_Scen",
             "L201.LaborForceFillout",
             "L201.LaborProductivity_gSSP3",
             "L201.PPPConvert"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c(XML = "socioeconomics_gSSP3.xml"))
    ...
    # Produce outputs
    create_xml("socioeconomics_gSSP3.xml") %>%
      add_xml_data(L201.Pop_gSSP3, "Pop_gSSP3") %>%
      add_xml_data(L201.BaseGDP_Scen, "BaseGDP_Scen") %>%
      add_xml_data(L201.LaborForceFillout, "LaborForceFillout") %>%
      add_xml_data(L201.LaborProductivity_gSSP3, "LaborProductivity_gSSP3") %>%
      ...
      socioeconomics_gSSP3.xml

    return_data(socioeconomics_gSSP3.xml)
  } else {
