#' module_socio_batch_SSP3.xml
#'
#' Construct XML data structure for \code{socioeconomics_SSP3.xml}.
#'
#' @param command API command to execute
#' @param ... other optional parameters, depending on command
#' @return Depends on \code{command}: either a vector of required inputs,
#' a vector of output names, or (if \code{command} is "MAKE") all
#' the generated outputs: \code{socioeconomics_SSP3.xml}. The corresponding file in the
#' original data system was \code{batch_socioeconomics_SSP3.xml} (socio XML).
module_socio_batch_SSP3.xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c("L201.Pop_SSP3",
             "L201.BaseGDP_Scen",
             "L201.LaborForceFillout",
             "L201.LaborProductivity_SSP3",
             "L201.PPPConvert"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c(XML = "socioeconomics_SSP3.xml"))
    ...
    # Produce outputs
    create_xml("socioeconomics_SSP3.xml") %>%
      add_xml_data(L201.Pop_SSP3, "Pop_SSP3") %>%
      add_xml_data(L201.BaseGDP_Scen, "BaseGDP_Scen") %>%
      add_xml_data(L201.LaborForceFillout, "LaborForceFillout") %>%
      add_xml_data(L201.LaborProductivity_SSP3, "LaborProductivity_SSP3") %>%
      ...
      socioeconomics_SSP3.xml

    return_data(socioeconomics_SSP3.xml)
  } else {
