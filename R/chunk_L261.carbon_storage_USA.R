...
module_gcam.usa_L261.carbon_storage_USA_DISABLED <- function(command, ...) {
 ...
 "L261.GlobalTechCoef_C"))
    ...
    return(c(XML = "L261.DeleteDepRsrc_USAC",
XML = "L261.DeleteSubsector_USAC",
XML = "L261.DepRsrc_FERC",
XML = "L261.DepRsrcCurves_FERC",
XML = "L261.SectorLogitTables_C_USA[[ curr_table ]]",
XML = "L261.Supplysector_C_USA",
XML = "L261.SubsectorLogitTables_C_USA[[ curr_table ]]",
XML = "L261.SubsectorLogit_C_USA",
XML = "L261.SubsectorShrwtFllt_C_USA",
XML = "L261.StubTech_C_USA",
XML = "L261.StubTechMarket_C_USA"))
...
tibble() %>%
 ...
 add_comments("comments describing how data generated") %>%
 add_comments("can be multiple lines") %>%
 add_legacy_name("L261.SectorLogitTables_C_USA[[ curr_table ]]") %>%
 add_precursors("precursor1", "precursor2", "etc") %>%
 # typical flags, but there are others--see `constants.R` 
 add_flags(FLAG_NO_TEST) %>%
  add_xml_data() ->
   L261.SectorLogitTables_C_USA[[ curr_table ]]
tibble() %>%
   add_title("descriptive title of data") %>%
 add_units("units") %>%
 add_comments("comments describing how data generated") %>%
...
tibble() %>%
 ...
 add_comments("comments describing how data generated") %>%
 add_comments("can be multiple lines") %>%
 add_legacy_name("L261.SubsectorLogitTables_C_USA[[ curr_table ]]") %>%
 add_precursors("precursor1", "precursor2", "etc") %>%
 # typical flags, but there are others--see `constants.R` 
 add_flags(FLAG_NO_TEST) %>%
  add_xml_data() ->
   L261.SubsectorLogitTables_C_USA[[ curr_table ]]
tibble() %>%
   add_title("descriptive title of data") %>%
 add_units("units") %>%
 add_comments("comments describing how data generated") %>%
 ...
 add_flags(FLAG_NO_TEST) %>%
  add_xml_data() ->
   L261.StubTechMarket_C_USA
    ...
    return_data(L261.DeleteDepRsrc_USAC, L261.DeleteSubsector_USAC, L261.DepRsrc_FERC, L261.DepRsrcCurves_FERC, L261.SectorLogitTables_C_USA[[ curr_table ]], L261.Supplysector_C_USA, L261.SubsectorLogitTables_C_USA[[ curr_table ]], L261.SubsectorLogit_C_USA, L261.SubsectorShrwtFllt_C_USA, L261.StubTech_C_USA, L261.StubTechMarket_C_USA)
  } else {
