...
module_aglu_LB151.ag_MIRCA_ctry_C_GLU_irr <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L100.LDS_ag_HA_ha %>%
      ...
      left_join(Unique_AGLU_ctry[ c( "iso", "CROSIT_country_ID") ], by = "iso" ) %>%                                    # Map in CROSIT country ids
      left_join(FAO_ag_items_PRODSTAT[ c("GTAP_crop","CROSIT_cropID")], by = "GTAP_crop" ) %>%                          # Map in CROSIT crop ids
      left_join(FAO_ag_CROSIT[ c( "country_ID", "crop_ID", "Yield_kgHa_irrigated", "Yield_kgHa_rainfed", "year" )],     # Map in CROSIT irrigated and rainfed yields
    ...
    ag_HA_ha_ctry_crop %>%
      left_join( Yieldratio_ctry_crop[ c( "iso", "GLU", "GTAP_crop", "yieldratio") ], by = c( "iso", "GLU", "GTAP_crop") ) %>%  # Map in the yield ratio
