...
module_emissions_L141.hfc_R_S_T_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # define function to formatt and process F gases, map to GCAM regions
    ...
    L141.EDGAR_HFC <- do.call(bind_rows,F_gases_formatted) %>%
    ...
    L141.hfc_R_S_T_Yh <- L141.hfc_R_S_T_Yh_share %>% left_join(L141.hfc_scaler,
                                        by = c("year", "Non.CO2")) %>% # there should be NA's, as not all sectors have emissions/scalers
