...
#' @details Regional supply curves were generated by summing up country-level supply curves and then smoothing them out.
#' @details Country-level supply curves were generated using data from Zhou. Every country within a region shared the
#' @details the same price points for which to calculate supply. The minimum price was determined to be the lowest base price
#' @details within that region. Maximum prices was calculated to be the price where supply is 95% that of the available resource.
#' @details The highest maximum price within the region was then used. The rest of the price points were filled out in equal increments.
...
module_energy_LA114.wind <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    generate_max_prices <- function(base.price, mid.price, curve.exponent) {
      ...
      L114.supply_tol <- 1 - 0.05
      max.price <- (L114.supply_tol/ (1-L114.supply_tol))^(1/curve.exponent) * mid.price + base.price
      max.price
    }
    ...
    if(OLD_DATA_SYSTEM_BEHAVIOR) {
      ...
      # Country-level supply curves from the Zhou file are used to buildup the regional supply curves
      Zhou_wind_supply_ctry_EJ %>%
        ...
        # Convert prices from 2007 USD to 1975 USD and energy from kWh to GJ
        mutate(mid.price = mid.price * CONV_2007_1975_USD / CONV_KWH_GJ,
               base.price = base.price * CONV_2007_1975_USD / CONV_KWH_GJ) ->
    ...
    # The price increment between min and max price is set at 0.1
    L114.price_inc <- 0.1
    ...
    if(OLD_DATA_SYSTEM_BEHAVIOR) {
      ...
      regional_price_supply_points <- data.frame()
    ...
    regional_price_supply_points <- lapply(regional_min_max_price_points$GCAM_region_ID, function(region_ID) {
      ...
      # Generates all the price points by establishing points at 0.1 increments from min to max
      ...
      # Note that the below code is an alternative way to acheive this
      # supply_points <- evaluate_smooth_res_curve(ctry_curves$curve.exponent, ctry_curves$mid.price, ctry_curves$base.price, ctry_curves$p)
    ...
    # 32 dataframes were created. Need to combine them all into one.
