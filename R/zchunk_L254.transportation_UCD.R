...
module_energy_L254.transportation_UCD <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    . <- UCD_technology <- Y <- addTimeValue <- base.service <-
      ...
      tranSubsector.x <- tranTechnology <- year <- year.fillout <- value <- NULL
    ...
    # First, create two lists to use to filter technologies
    L254.StubTranTech %>%
    ...
    if(any(!is.na(A54.tranSubsector_shrwt$year))){
      A54.tranSubsector_shrwt %>%
        ...
        L254.tranSubsectorShrwt # OUTPUT (followup...this exists?)
    }
    ...
    A54.globaltranTech_interp %>%
      mutate(from.year = replace(from.year, from.year == "final-calibration-year", max(BASE_YEARS)),
             to.year = replace(to.year, to.year == "end-year", max(FUTURE_YEARS))) %>%
      rename(sector.name = supplysector, subsector.name = tranSubsector) %>%
    ...
    L154.loadfactor_R_trn_m_sz_tech_F_Y %>%
      filter(year %in% MODEL_YEARS) %>% # did not interpolate
    ...
    CONV_EJ_GJ <- 1e9 # followup delete
    ...
    L254.StubTranTechCalInput %>%
      ...
      select(region, supplysector, tranSubsector, stub.technology, year, minicam.energy.input = minicam.energy.input.x,
             calibrated.value, loadFactor, coefficient, output) ->
      L254.StubTranTechOutput # followup....sum is 533001370 instead of 533001243
    ...
    A54.globaltech_passthru %>%
      ...
      filter(r_ss_ts_st %in% LIST_r_ss_ts_st) %>%
      select(-r_ss_ts_st) %>%
      rename(calibrated.value = output) %>%
      mutate(share.weight.year = year,
             subs.share.weight = if_else(calibrated.value > 0, 1, 0),
             tech.share.weight = if_else(calibrated.value > 0, 1, 0)) ->
      L254.StubTechCalInput_passthru # OUTPUT followup a mess, need to fix a few things
    ...
    L254.StubTechCalInput_passthru_cum %>%
      #select(region, supplysector, tranSubsector, stub.technology, year,
             #minicam.energy.input, output_agg, output_cum) %>%
...
#     }
    ...
    #A54.globaltech_passthru, GCAM_region_names, L254.StubTranTech_names_StubTranTech_followup
    ...
    L254.Supplysector_trn %>%
      ...
      add_precursors("common/GCAM_region_names", "energy/mappings/UCD_techs", "L154.intensity_MJvkm_R_trn_m_sz_tech_F_Yr",
    ...
    L254.tranSubsectorVOTT %>%
      ...
      add_precursors("common/GCAM_region_names", "energy/A54.tranSubsector_VOTT",
                     "energy/mappings/UCD_techs", "L154.intensity_MJvkm_R_trn_m_sz_tech_F_Yr",
    ...
    L254.GlobalRenewTech_nonmotor %>%
      ...
      add_precursors("energy/54.globaltech_nonmotor") ->
      L254.GlobalRenewTech_nonmotor
    ...
    L254.StubTechCalInput_passthru %>%
      ...
      add_units("Calibrate.value in_followup") %>%
      ...
      add_precursors("common/GCAM_region_names", "energy/mappings/UCD_techs",
                     "L154.intensity_MJvkm_R_trn_m_sz_tech_F_Yr", "A54.globaltech_passthru") ->
      L254.StubTechCalInput_passthru
    ...
    L254.BaseService_trn %>%
      ...
      add_precursors("L254.StubTranTechOutput", "L254.StubTechProd_nonmotor", "A54.sector") ->
      L254.BaseService_trn
