...
#' the generated outputs: \code{L120.RsrcCurves_EJ_R_offshore_wind}, \code{L120.TechChange_offshore_wind}, \code{L120.GridCost_offshore_wind},
#' \code{L120.RegCapFactor_offshore_wind}. The corresponding file in the
#' original data system was \code{LA120.offshore_wind.R} (energy level1).
#' @details Takes in data on country-level offshore wind energy potential and global offshore wind capital cost assumptions and generates tables containing region-level offshore wind data.
...
module_energy_LA120.offshore_wind <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    resource.potential.EJ <- fixed.charge.rate <- technology <- X_final_historical_year <- fcr <- OM.fixed <- price <- CFmax <- supply <- curve.exponent <-
    mid.price <- Pmin <- maxSubResource <- supply_points <- Pvar <- P1 <- Q1 <- sector.name <- supplysector <- subsector.name <- subsector <-
    intermittent.technology <- year <- input.capital <- capital.overnight <- capacity.factor <- capital.overnight.lag <- capital.tech.change.5yr <- kl  <-
    tech.change.5yr <- tech.change <- bin <- cost <- Wind_Resource_Region <- Wind_Class <- grid.cost <- NULL  # silence package check notes
	...
	A23.globaltech_OMfixed %>%
	  gather_years() %>%
	  filter(technology == "wind_offshore", year == max(HISTORICAL_YEARS)) %>%
	...
	L120.offshore_wind_matrix %>%
	  group_by(GCAM_region_ID) %>%
	  arrange(GCAM_region_ID, price) %>%
	  mutate(Pmin = min(price), Pvar = price - Pmin, maxSubResource = round(max(supply),energy.DIGITS_MAX_SUB_RESOURCE)) %>%
	...
	L120.mid.price_1 %>%
	  ...
	  mutate(mid.price = round(((P2-P1)*maxSubResource + 2*Q2*P1 - 2*Q1*P2)/(2*(Q2-Q1)),energy.DIGITS_MAX_SUB_RESOURCE)) %>%
	...
	L120.offshore_wind_cap_cost %>%
	  ...
	  mutate(k1 = fixed.charge.rate/ (CONV_YEAR_HOURS*CONV_KWH_GJ), k2 = 1/(CONV_YEAR_HOURS*CONV_KWH_GJ)) %>%
	  mutate(tech.change.5yr = (k1* capital.tech.change.5yr * capital.overnight + k2*OM.fixed)/(k1*capital.overnight + k2*OM.fixed)) %>%
	  mutate(tech.change = round(abs(1-(tech.change.5yr)^(1/5)),energy.DIGITS_MAX_SUB_RESOURCE)) %>%
	  select(year, tech.change) %>%
	  filter(is.na(tech.change) == "FALSE") %>%
	  filter(year > max(MODEL_BASE_YEARS)) -> L120.TechChange_offshore_wind
	...
	offshore_wind_grid_cost %>%
	  mutate(distance= cut(rep_dist_for_bin, breaks=c(0, NREL_wind_energy_distance_range$max), right=FALSE, labels= NREL_wind_energy_distance_range$distance)) -> offshore_wind_grid_cost

	# Add representative distances and cost to the potential share tables and calculate total cost by region.
	L120.offshore_wind_potential_share %>%
	  left_join_error_no_match(offshore_wind_grid_cost %>%
	         select(-technology),
	         by = c("distance_to_shore" = "distance")) %>%
	  select(-distance_to_shore,-total) %>%
	  rename(cost_per_kW_km='cost_per_kW-km') %>%
	  mutate(cost= share*cost_per_kW_km*rep_dist_for_bin) %>%
	  group_by(region) %>%
    mutate(cost=sum(cost)) %>%
	  select(region,cost) %>%
	  unique() -> L120.offshore_wind_cost_per_km

	# Now to convert 2013 $/km cost to 1975 $/GJ cost
	L120.offshore_wind_cost_per_km %>%
	  left_join_error_no_match(L120.offshore_wind_CF %>%
	         select(region,CF) %>%
	         unique(),
	         by =c("region")) %>%
	  mutate(fcr = L120.offshore_wind_fcr) %>%
	  mutate(grid.cost = fcr*cost/(8760*CF*CONV_KWH_GJ)) %>%
	  mutate(grid.cost = grid.cost*gdp_deflator(1975,2013)) -> L120.grid.cost
    ...
    GCAM_region_names %>%
      ...
      left_join_error_no_match(L120.grid.cost %>%
	           select(region,grid.cost),
						 by=c("region")) -> L120.GridCost_offshore_wind
    ...
    L120.RsrcCurves_EJ_R_offshore_wind %>%
      ...
      add_precursors( "common/iso_GCAM_regID", "common/GCAM_region_names", "energy/NREL_offshore_energy",  "energy/A20.wind_class_CFs", "energy/A23.globaltech_capital", "energy/A23.globaltech_OMfixed", "energy/A20.offshore_wind_depth_cap_cost") ->
      L120.RsrcCurves_EJ_R_offshore_wind
    ...
    L120.GridCost_offshore_wind %>%
      ...
      add_precursors("common/iso_GCAM_regID", "common/GCAM_region_names", "energy/NREL_offshore_energy",  "energy/A20.wind_class_CFs", "energy/A23.globaltech_capital", "energy/offshore_wind_grid_cost", "energy/NREL_wind_energy_distance_range") ->
      L120.GridCost_offshore_wind
    ...
    return_data(L120.RsrcCurves_EJ_R_offshore_wind, L120.TechChange_offshore_wind, L120.GridCost_offshore_wind, L120.RegCapFactor_offshore_wind)
  } else {
