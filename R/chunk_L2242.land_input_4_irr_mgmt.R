...
module_aglu_L2242.land_input_4_irr_mgmt_DISABLED <- function(command, ...) {
 ...
 "L2241.LN4_NodeIsGhostShareRel"))
    ...
    return(c(XML = "L2241.LN4_LogitTables[[ curr_table_name ]]",
XML = "L2242.LN4_Logit",
XML = "L2242.LN4_NodeGhostShare",
XML = "L2242.LN4_NodeIsGhostShareRel"))
 ...
 add_legacy_name("L2241.LN4_LogitTables[[ curr_table_name ]]") %>%
 add_precursors("precursor1", "precursor2", "etc") %>%
 # typical flags, but there are others--see `constants.R` 
 add_flags(FLAG_NO_TEST) %>%
  add_xml_data() ->
   L2241.LN4_LogitTables[[ curr_table_name ]]
tibble() %>%
   add_title("descriptive title of data") %>%
 add_units("units") %>%
 add_comments("comments describing how data generated") %>%
 ...
 add_flags(FLAG_NO_TEST) %>%
  add_xml_data() ->
   L2242.LN4_NodeIsGhostShareRel
    ...
    return_data(L2241.LN4_LogitTables[[ curr_table_name ]], L2242.LN4_Logit, L2242.LN4_NodeGhostShare, L2242.LN4_NodeIsGhostShareRel)
  } else {
