...
#' the generated outputs: \code{L102.unlimited_water_price_state_R_W_Y_75USDm3}. The corresponding file in the
#' original data system was \code{L102.water.supply.unlimited.R} (water level1).
#' @details  We generate all the possible combinations between the 50 GCAM-USA states, 4 water types, and all the defined years.
...
module_gcamusa_L102.water.supply.unlimited_USA <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    return(c("L102.unlimited_water_price_state_R_W_Y_75USDm3"))
  } else if(command == driver.MAKE) {


    all_data <- list(...)[[1]]

    region <- year <- water_type <- value <- output.unit <- NULL                      # silence package check.
    ...
    # all 4 water types
    water_type <- c("water consumption", "water withdrawals", "seawater", "biophysical water consumption","desalination")
    ...
    L102.all_state_water_type %>%
      ...
      add_precursors("gcam-usa/states_subregions", "water/A_unlimited_water_price") ->
      L102.unlimited_water_price_state_R_W_Y_75USDm3

    return_data(L102.unlimited_water_price_state_R_W_Y_75USDm3)
  } else {
