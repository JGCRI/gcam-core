...
module_energy_L2231.wind_update <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    country_name <- GCAM_region_ID <- IAM.Country.Name <- wind_class <- resource.potential.EJ <- region <-
      distance <- X_final_model_base_year <- fixed.charge.rate <- capital.overnight <-
      fcr <- OM.fixed <- CFmax <- price <-
      supply <- Pmin <- Pvar <- maxSubResource <- percent.supply <- P1 <- Q1 <- mid.price <- P2 <- Q2 <- curve.exponent <-
      ...
      techChange <- minicam.energy.input <- input.cost <- NULL  # silence package check notes
    ...
    A23.globaltech_capital %>%
      gather_years() %>%
      filter(technology == "wind", year == max(MODEL_BASE_YEARS)) %>%
    ...
    L2231.onshore_wind_cap_cost %>%
      ...
      mutate(tech.change = round(abs(1-(tech.change.5yr)^(1/5)),energy.DIGITS_COST)) %>%
      select(year, tech.change) %>%
      filter(is.na(tech.change) == "FALSE") %>%
      filter(year > max(MODEL_BASE_YEARS)) -> L2231.TechChange_onshore_wind
  ...
  # Grid connection costs are read in as fixed non-energy cost adders (in $/GJ). This is calculated using three things: 1. the average onshore wind $/kW-km data
	# 2. representative distances from bins used by NREL to assess wind potential and 3. Share of wind potential at various distance bins.
	...
	# which is then converted to $/GJ, and the final number is obtained for each region by using the apporpriate FCR and deflator. The representative distance for each
	# bin are respectively 25 miles (40 km), 75 miles (120 km) and 100 miles (160 km) - for bins of 0-50 miles, 50-100 miles, and 100+ miles.
	# The literature generally assumes a 25-75 mile grid distance for onshore wind projects, so taking midpoints of the first two bins seems reasonable, while there are
	# only a few projects developed beyond a 100 mile distance from the grid, so taking the minimum point for that bin makes sense.
	...
	onshore_wind_grid_cost %>%
	  mutate(distance= cut(rep_dist_for_bin, breaks=c(0, NREL_wind_energy_distance_range$max), right=FALSE, labels= NREL_wind_energy_distance_range$distance)) -> onshore_wind_grid_cost
    ...
    mutate(cost=sum(cost)) %>%
	  select(region,cost) %>%
	  unique() -> L2231.onshore_wind_cost_per_km

	# Now to convert 2010 $/km cost to 1975 $/GJ cost
	L2231.onshore_wind_cost_per_km %>%
	  ...
	  mutate(grid.cost = fcr*cost/(8760*capacity.factor*CONV_KWH_GJ)) %>%
	  mutate(grid.cost = grid.cost*gdp_deflator(1975,2010)) -> L2231.grid.cost
    ...
    GCAM_region_names %>%
      ...
      left_join_error_no_match(L2231.grid.cost %>%
                                 ...
                                 by=c("region")) -> L2231.GridCost_onshore_wind
    ...
    L2231.onshore_wind_curve %>%
      mutate(renewresource = "onshore wind resource", smooth.renewable.subresource = "onshore wind resource", year.fillout=min(MODEL_BASE_YEARS)) %>%
