...
double BuildingGompertzFunction::calcCoefficient( InputSet& input, double consumption, const std::string& regionName,
...
double BuildingGompertzFunction::calcDemand( InputSet& input, double income, const std::string& regionName,
                       ...
                       double capitalStock, double alphaZero, double sigma, double IBT, const IInput* aParentInput ) const
{
    ...
    for( InputSet::iterator inputIter = input.begin(); inputIter != input.end(); ++inputIter ) {
        ...
        // Use the gompertz to calculate per capita demand.
        double perCapitaDemand =
            buildingNodeInput->getGompertzDemandFunction()->calcDemand( buildingNodeInput->getSubregionalIncome() * priceTerm );
...
double BuildingGompertzFunction::calcLevelizedCost( const InputSet& aInputs, const std::string& aRegionName,
