...
test_that("change_iso_code works", {
  ...
  d <- tibble(iso=c('dum','bad', 'bad', 'bad', 'dum'), `2005`=rep(23,5))
...
test_that('protect and unprotect integer cols work', {
  d <- tibble(iso=c('bad','dum'), `2005`=c(123.45, NA), `2050`=c(867, 5309))
  d2 <-protect_integer_cols(d)
  expect_equal(names(d2), c('iso','X2005','X2050'))
  d2 %>% dplyr::select_if(function(col){!any(is.na(col))}) %>% unprotect_integer_cols -> d3
