...
context("Test CSV to XML pipeline utilities")
...
test_that("bogus MI header causes error", {
  skip("TODO: need to find a way to get messages from Java")
...
test_that("can convert single table", {
  ...
  test_conn <- file(test_fn, "r")
  test_xml <- readLines(test_conn)
  close(test_conn)
  unlink(test_fn)
  test_xml <- test_xml %>%
    gsub("^\\s+|\\s+$", "", .) %>%
...
test_that("get warning for missing header", {
  ...
  create_xml(test_fn) %>%
    add_xml_data(data1, "InterestRate") %>%
    add_xml_data(data2, "Will_Not_Find") -> conv_test
  # TODO: need to find a way to get messages from Java
  #expect_message(run_xml_conversion(conv_test), regex="Warning: skipping table: Will_Not_Find!", all=F)
